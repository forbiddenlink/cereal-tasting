    ...
    return (
        ...
        >
            <motion.div
                ...
                className="relative bg-merlot-dark/50 backdrop-blur-sm border border-gold/20 rounded-xl p-6 shadow-2xl transition-shadow duration-500 group-hover:shadow-[0_0_30px_rgba(212,175,55,0.2)]"
            >
                {/* Vintage Badge */}
                <div className="absolute -top-3 -right-3 z-20 bg-gold text-merlot font-heading font-bold px-3 py-1 rounded-full shadow-lg transform rotate-12 group-hover:rotate-0 transition-transform">
                    Est. {cereal.vintage}
                </div>

                {/* Cereal Box Image */}
                <div
                    className="relative w-full max-w-[260px] mx-auto aspect-[3/4] mb-6 transform-style-3d group-hover:translate-z-10 transition-transform duration-500"
                ...
                <div className="text-center transform-style-3d" style={{ transform: "translateZ(10px)" }}>
                    <h3 className="text-2xl font-heading text-gold mb-1 group-hover:text-cream transition-colors">
                        {cereal.name}
                    </h3>
                    <p className="text-sm font-mono text-zinc-400 mb-4">{cereal.region}</p>

                    <div className="flex justify-center gap-2 mb-4">
                        {cereal.tastingNotes.slice(0, 2).map((note, i) => (
                            <span key={i} className="text-xs bg-white/5 border border-white/10 px-2 py-1 rounded-full text-cream/70">
                                {note}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="flex justify-between items-center border-t border-white/10 pt-4 mt-2">
                        <div className="text-left">
                            ...
                            <p className="font-mono text-slime">${cereal.price.toFixed(2)}</p>
                        </div>
                        <button 
                            onClick={handleAddToCart}
                            className="relative bg-gold text-merlot px-4 py-2 rounded-md font-bold text-sm hover:bg-cream transition-colors overflow-hidden"
                        >
                            {addedToCart ? (
                                <motion.span
                    ...
                    <div className="flex justify-between items-center border-t border-white/10 pt-4 mt-2">
                        ...
                        <button className="bg-gold text-merlot px-4 py-2 rounded-md font-bold text-sm hover:bg-cream transition-colors">
                            Taste
                        </button>
                    </div>
                </div>
