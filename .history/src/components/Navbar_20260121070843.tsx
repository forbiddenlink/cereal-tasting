    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 py-4 px-4 md:px-6 glass-panel-heavy">
            <div className="max-w-7xl mx-auto flex justify-between items-center">
                ...
                >lg md:text-xl lg:text-2xl font-heading text-gold tracking-wider uppercase group-hover:text-cream transition-colors duration-300">
            ...
            </AnimatePresence           onClick={() => onNavigate(item.id as 'home' | 'pairing' | 'about')}
                            className={`
                                relative py-2 transition-all duration-300 hover:text-white
                                ${currentPage === item.id ? 'text-gold' : ''}
                            `}
